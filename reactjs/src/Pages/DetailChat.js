...
const DetailChat = () => {
  ...
  return (
      <div style={{ display: "flex", height: "100%" , width: "100%"}}>
        <div id={style.TabChatDetail} style={{width: "25%"}}>
          <TabChat />
        </div>
        <div style={{ width: !extend ? `75%` : `50%` }} id="layoutChatRoom">
        <div
          style={{
            display: "flex",
        ...
        >
          <ChatHeader chat={chat} setExtend={setExtend} extend={extend}/>
          <ListMessage
            reply={reply}
            setReply={setReply}
            messages={messages}
      ...
      {extend && <div id="layoutInforUser" style={{ padding: "15px 12px" }}>
        <InforUser chat={chat} />
      </div>}
      </div>
  );
